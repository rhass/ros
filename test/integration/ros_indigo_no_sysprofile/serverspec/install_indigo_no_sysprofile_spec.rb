...
describe 'install_indigo_no_sysprofile' do
  it 'installs ROS indigo' do
    expect(package('ros-indigo-ros-base')).to be_installed
  end

  it 'does not add a symlink to the ros indigo setup.sh in /etc/profiles.d' do
    expect(file('/etc/profile.d/indigo.sh')).not_to be_symlink
    expect(file('/etc/profile.d/indigo.sh')).not_to be_linked_to('/opt/ros/indigo/setup.sh')
  end
